      ...
      def apply(elements: FieldElements) = {
        ...
        Html(<div class={s"control-group ${elements.args.get('_class).getOrElse("")} ${error}"}
          ...
          <div class="form-group">
            <label class="control-label col-lg-3" for={elements.id}>{elements.args.get('_label).getOrElse(Config.translator.field(m.runtimeClass, elements.field.name)(elements.p.messages.lang.toLocale))}</label>
            <div class="col-lg-5">
